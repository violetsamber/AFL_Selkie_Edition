...
private _respiratoryDepth = DEFAULT_RESPIRATORY_DEPTH;
...
private _pao2 = ((_previousCyclePao2 - (DEFAULT_ECB / ((GET_BODY_FLUID(_unit) select 0) min 500)) * ((_demandVentilation - _actualVentilation) / 120)));

private _string = format["paO2: %1   pALVo2: %2",_pao2, _pALVo2];
LOG(_string);
...
_pao2 = if (_previousCyclePao2 != _pao2) then {
    ([ (_previousCyclePao2 - ((PAO2_MAX_CHANGE * EGVAR(breathing,SpO2_MultiplyNegative) * _arrestPerfusion) * _deltaT)) , (_previousCyclePao2 + ((PAO2_MAX_CHANGE * EGVAR(breathing,SpO2_MultiplyPositive)) * _deltaT))] select ((_previousCyclePao2 - _pao2) < 0)) 
} else { 
    _pao2 
};
